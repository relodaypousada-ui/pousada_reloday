...
import { PacoteMidia } from "@/integrations/supabase/pacotes";
...
interface PacoteMediaManagerProps {
  pacoteId: string;
  initialMedia: PacoteMidia[];
}
...
const createMedia = async (newMedia: MediaInsert): Promise<PacoteMidia> => {
    ...
    if (error) {
        console.error("Erro ao adicionar mídia ao pacote:", error);
        throw new Error(`Falha ao adicionar mídia: ${error.message}`);
    }
    return data as PacoteMidia;
};
...
const deleteMedia = async (id: string): Promise<void> => {
    ...
    if (error) {
        console.error("Erro ao deletar mídia do pacote:", error);
...
const updateMediaOrder = async (mediaUpdates: { id: string, ordem: number }[]): Promise<void> => {
    ...
    if (error) {
        console.error("Erro ao atualizar ordem da mídia do pacote:", error);
...
const PacoteMediaManager: React.FC<PacoteMediaManagerProps> = ({ pacoteId, initialMedia }) => {
  const queryClient = useQueryClient();
  const [mediaList, setMediaList] = useState<PacoteMidia[]>(initialMedia);
  ...
  // Storage Hook
  const uploadPath = useMemo(() => `pacotes/${pacoteId}`, [pacoteId]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pacotes", pacoteId] });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl">Galeria de Mídias Adicionais do Pacote</CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          {uploadMethod === 'url' ? (
            ...
            <label htmlFor="pacote-media-upload" className="flex-1">
...
export default PacoteMediaManager;